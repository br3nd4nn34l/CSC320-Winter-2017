...
home = "home"
cdf = "cdf"
submission_folder = make_path(["Submission", "A3"])
...
def get_location_based_path(location):
    ...
    A3_folder = make_path([cur_CS_work_path, "CSC320-Winter-2017", "Assignments", "Assignment 3"])
    return A3_folder
...
if __name__ == '__main__':
    ...
    A3_folder = get_location_based_path(location)
    submission_folder = submission_path(A3_folder)
    ...
    source_to_dest = {
          # The checklist
          make_path([A3_folder, instruction_folder, "CHECKLIST.txt"]):
          make_path([submission_folder, "CHECKLIST.txt"]),

          # Code folder
          make_path([A3_folder, "Code"]):
          make_path([submission_folder, "code"]),

          # Report folder
          make_path([A3_folder, "Report"]):
          make_path([submission_folder, "report"]),

          # Extras
          make_path([A3_folder, "Extras"]):
