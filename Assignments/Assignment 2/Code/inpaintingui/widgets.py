...
# Kivy's built-in FloatLayout widget class
...
#########################################
    ...
    # Run the algorithm associated with the GUI's current mode UNTIL IT IS COMPLETED
    def run_algorithm(self):
        # the algorithm is executed by calling the method of
        # the InpaintingControl class

        # Set the maxIterations = None to indicate that the algorithm should run until full completion
        ok, msg = self.inpaintingControl.runAlgorithm(self.ids.imviewer, maxIterations=None)
